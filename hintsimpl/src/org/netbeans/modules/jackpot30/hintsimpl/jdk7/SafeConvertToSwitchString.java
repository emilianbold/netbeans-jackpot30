...
public class SafeConvertToSwitchString {
    ...
    public static List<ErrorDescription> hint(HintContext ctx) {
        Preferences prefs = RulesManager.getPreferences("org.netbeans.modules.java.hints.jdk.ConvertToStringSwitch", HintsSettings.getCurrentProfileId());
        boolean old = prefs.getBoolean(KEY_ALSO_EQ, DEF_ALSO_EQ);

        prefs.putBoolean(KEY_ALSO_EQ, false);

        try {
            return ConvertToStringSwitch.hint(ctx);
        } finally {
            prefs.putBoolean(KEY_ALSO_EQ, old);
        }
    }
